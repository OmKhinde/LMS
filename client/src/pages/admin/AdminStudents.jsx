...
const AdminStudents = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.warn('Failed to load students', err?.response?.data || err.message);
        setItems([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-4">
        <input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search students..." className="border px-3 py-2 rounded w-full max-w-md" />
      </div>
      {loading ? (
            ...
            <tbody>
              {pageItems.map(u => (
                <tr key={u._id || u.id} className="border-t">
                  ...
                  <td className="p-3">{u.enrolledCourses?.length || 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div className="p-3 flex items-center justify-between">
            <div className="text-sm text-gray-600">{filtered.length} students</div>
